...
| 2 | Hero (`#home`) | 다크 | 텍스트 H1(메인 카피, "설계" 그라데이션) + **오비탈 시스템 코어**(동심 링 3·스윕 아크·위성·코어 펄스, 70%W·42%H). 데스크탑: **260svh 핀 "Signal Fly-Through" 4막** — 카피 침강→필드 정렬(층류)→방사 줌 진입(링 순차 게이트 통과)→코어 플레어·레일 스트릭·Signal Strip 오버랩 바통터치(design.md §17). 모바일: 앰비언트 자전 + 14s 정렬 호흡. 포인터 잔물결 반응 |
...
| **v10** | **히어로 재구성 "Order from Noise"**: 콘솔 카드 제거(죽은 CSS ~140줄 포함 정리), 파티클 단일 서사(산포→4레인 정렬→좌향 동기화→레일 융합), 레인 라벨·가이드, 브리지 무대 중앙 이동, 모바일 자율 호흡 사이클. v10.1: 오비탈 시스템 코어 추가 | §16 |
| **v11** | **스크롤 안무 전면 재설계 "Signal Fly-Through"**: 수렴(중앙으로 모임) 폐기 → 진입(방사 줌·링 순차 게이트 통과·코어 플레어→레일 스트릭). "한 막 = 한 지배 모션" 원칙, 레인·라벨·위상분리 삭제, 무대 scale 제거로 레일 정합 단순 산술화(델타 0.0px) | §17 |
