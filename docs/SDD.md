...
| 2 | Hero (`#home`) | 다크 | 텍스트 H1(메인 카피, "설계" 그라데이션) + **기하학 파티클 서사**(콘솔 카드 제거 — 제품 목업은 #solution 전담). 데스크탑: **260svh 핀 스크럽** — 카피 침강→파티클 4레인 정렬(Ads·Data·CRM·Commerce 라벨)→좌향 동기화→레일 융합→Signal Strip 오버랩 바통터치(design.md §15·§16). 모바일: 산포⇄정렬 14s 자율 호흡. 포인터 잔물결 반응 |
...
| **v10** | **히어로 재구성 "Order from Noise"**: 콘솔 카드 제거(죽은 CSS ~140줄 포함 정리), 파티클 단일 서사(산포→4레인 정렬→좌향 동기화→레일 융합), 레인 라벨·가이드, 브리지 무대 중앙 이동, 모바일 자율 호흡 사이클 | §16 |
